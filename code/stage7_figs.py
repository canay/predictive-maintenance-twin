...
ONLY = os.environ.get("FH8_ONLY_FIGURE", "").strip()

def pending(name):
    return (not ONLY or ONLY == name) and (bool(ONLY) or not done(name))
...
if pending("fig_calibration"):
    ...
    for i in range(5):
        ...
        ax.hist(real[i], bins=b, density=True, alpha=0.55, color=OI[0], label="AI4I")
    ...
    ax.bar(xm - w / 2, rs, w, color=OI[0], label="AI4I")
